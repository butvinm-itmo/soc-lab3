...
Циклы, развернутые в параллельные операции.


<img src="./assets/unroll.png" width="1000"/>
...
Внутренний цикл k полностью развернут, что позволило выполнять 7 умножений параллельно. Однако, полученная схема не укладывается в необходимый тайминг 10ns.

Исправим это, указав параметр `factor` и тем самым снизим количество операций, выполняемых за один такт.

## Partially Unrolled Loops (#pragma UNROLL factor=2)

Частичное развертывание цикла - компромисс между полным UNROLL и baseline. За один такт выполняется 2 итерации внутреннего цикла вместо 7.
...
| Latency (cycles) | 1374                              |
| Clock Period     | 10 ns                             |
| **Elapsed time** | **13.74 мкс**                     |
| Estimated Clock  | 9.332 ns (укладывается в target!) |

### Структура циклов

| Цикл           | Latency | Iteration Latency | Trip Count | Pipelined |
| -------------- | ------- | ----------------- | ---------- | --------- |
| Loop 1 (i)     | 1288    | 184               | 7          | No        |
| Loop 1.1 (j)   | 182     | 26                | 7          | No        |
| Loop 1.1.1 (k) | 24      | 7                 | 3\*        | No        |
| Loop 2 (i)     | 84      | 12                | 7          | No        |
...
Цикл раскладывается в пайплайн, где несколько итераций накладываются по времени за счет исполнения _различных_ операций в один такт - аналогично пайплайну в CPU - за счет чего достигается параллелизм.
...
Циклы были объединены конвейеризированы. II=14 означает, что новая итерация начинается каждые 14 тактов, II=1 не достигнут из-за зависимостей по данным.
...
Внутренний цикл `for k` выполняет 7 умножений-сложений последовательно, каждая операция занимает ~2 такта (чтение из памяти + вычисление). 7 × 2 = 14 тактов.
...
| -------------------- | -------- | ------ | -------------- | -------- |
| **Latency (cycles)** | 1598     | 611    | 1374           | 741      |
| **Время (мкс)**      | 15.98    | 6.11   | 13.74          | 7.41     |
| **Ускорение**        | 1.0x     | 2.6x   | 1.16x          | 2.2x     |
| DSP48E               | 1        | 4      | 2              | 4        |
| FF                   | 450      | 932    | 529            | 563      |
| LUT                  | 602      | 1134   | 714            | 1081     |
| Est. Clock (ns)      | 7.18     | 11.00  | 9.33           | 10.86    |
...
3. **Partial Unroll (factor=2)** - исправленный UNROLL:
   - Умеренное ускорение (1.16x)
   - Timing выполнен (9.33 ns < 10 ns target)
   - Минимальный рост ресурсов (DSP: 2x, LUT: 1.2x)

4. **Pipeline** - лучший компромисс:
   - Существенное ускорение (2.2x)
   - Умеренный рост ресурсов
   - Timing близок к target, но может потребовать оптимизации

**Рекомендация:** Для данного проекта оптимальным выбором является **Pipeline** версия, так как она обеспечивает максимальное ускорение при соблюдении временных ограничений. **Partial Unroll** - хороший выбор если нужна гарантия timing с запасом.
...
### Partial Unroll версия (mat_partial_unroll.c)

```c
#include "mat.h"

// PARTIAL UNROLL VERSION: Inner loops partially unrolled (factor=2)
// Computes C = A + B*B
...
Были реализованы и сравнены четыре варианта оптимизации:

1. **Baseline** - без директив оптимизации, последовательное выполнение всех операций
2. **Unroll** - полное развертывание внутренних циклов для параллельного выполнения умножений
3. **Partial Unroll (factor=2)** - частичное развертывание для исправления timing violation
4. **Pipeline** - конвейеризация циклов для перекрытия итераций

Результаты показали, что:

- Директива UNROLL обеспечивает максимальное ускорение (2.6x), но приводит к нарушению временных ограничений (11 ns > 10 ns target)
- Директива UNROLL с параметром factor=2 исправляет timing violation (9.33 ns), но даёт меньшее ускорение (1.16x)
- Директива PIPELINE обеспечивает лучший баланс между производительностью (2.2x ускорение) и соблюдением временных ограничений
- Все варианты используют менее 2% ресурсов FPGA, что позволяет интегрировать ускоритель в более сложные системы

Для реального применения рекомендуется использовать **Pipeline** версию как оптимальный компромисс между производительностью и надежностью. **Partial Unroll** - альтернатива с гарантированным запасом по timing.
